// 信用卡管理 Cloudflare Worker（前端 SPA + D1 API + scheduled 推送）
// 简要说明：提供单页前端用于管理信用卡（账单日/还款日查询、导出、管理员增删改查）
// 后端暴露简单 API：/api/login, /api/cards, /api/cards/:id（GET/POST/PUT/DELETE）
// scheduled 用于定时检查并通过 PushPlus 推送“剩余1天还款”提醒
// 务必先修改第一行的ADMIN_TOKEN 的值。
// worker.js-V3.3.20260102

const ADMIN_TOKEN = "strong-secret-admin-token-54321";
    ...
    try {
      ...
      const contentHtml = `
        <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial;">
          <h2 style="margin:0 0 8px 0;font-size:16px;">信用卡还款提醒（剩余 1 天）</h2>
          ${listHtml}
...
<body class="bg-white text-gray-900">
    ...
    <script>
        // --- 客户端脚本（与原实现一致，关键修复：renderCalendar 中正确计算上个月月份/年份） ---
        let allCards = [];
        let filteredCards = [];
        ...
        function renderCalendar(date) {
            ...
            for (let day = 1; day <= daysInMonth; day++) {
                ...
                if (calendarMode === 'billingDay' && isBilling) {
                    ...
                    dayEl.classList.add('opacity-50', 'bg-green-50');
                } else if (isPayment) {
                    dayEl.classList.add('opacity-50', 'bg-red-50');
                }
