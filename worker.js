...
// V3版本新增微信提醒功能，需要自行添加环境变量PUSHPLUS_API、PUSHPLUS_TOKEN。
// 定义一个简单的静态令牌用于管理员认证，请自行修改
const ADMIN_TOKEN = "strong-secret-admin-token-12345";
...
export default {
  ...
  // 新增：scheduled 处理器，用于 Cron Trigger 调度
  // 当在 Cloudflare Dashboard 中配置 Cron Trigger 指向此 Worker 时会调用
  async scheduled(event, env, ctx) {
    // 使用 waitUntil 以便在后台执行异步任务
...
// =============================================
// 新增部分（仅用于每天定时检测并通过 PushPlus 推送“剩余1天还款”提醒）
// 说明：我只在文件中新增以下代码，未修改原有任何逻辑或实现。
// 请在 Cloudflare Worker 设置中添加环境变量：PUSHPLUS_TOKEN（必需），可选 PUSHPLUS_API（默认为 https://www.pushplus.plus/send）
// 并在 Dashboard > Triggers 创建一个 Cron Trigger（例如 0 1 * * *）以触发 scheduled。
// =============================================

/**
 * 服务器端：根据账单日计算还款截止日（防止字段为字符串，使用 Number 强制转换）
 * billingDate: Date
 * paymentType: 'days_after_billing' | 'fixed_day'
...
function calculatePaymentDeadlineServer(billingDate, paymentType, paymentValue, graceDays) {
  ...
  // 加上宽限期
  paymentDate.setDate(paymentDate.getDate() + gd);
  return paymentDate;
}

/**
 * 服务器端：获取卡片下次账单与下次付款截止日信息
 * 返回 { nextBillingDate, nextPaymentDeadline, daysUntilPayment }
 */
...
/**
 * 主任务：读取 D1 中所有卡片，筛选出 daysUntilPayment === 1 的卡片，
 * 如果存在则调用 PushPlus 接口发送推送（标题："信用卡还款"，内容示例："招商银行(1234)、工商银行(5678) 信用卡还款日还剩余1天，请注意还款"）
 */
async function doScheduledPush(env) {
  try {
    ...
    // 2. 构造推送标题与内容
    const banksText = dueSoon.map(item => {
      const c = item.card;
      const tail = c.last_4_digits ? String(c.last_4_digits) : '';
      return c.bank_name + (tail ? `(${tail})` : '');
    }).join('、');

    const title = '信用卡还款';
    const content = `${banksText} 信用卡还款日还剩余1天，请注意还款`;
